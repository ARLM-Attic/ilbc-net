...
namespace iLBC {
    ...
    public class ilbc_encoder {
        ...
        private void AbsQuantW(
            ...
            int weightDenum_idx,
            int[] _out,           /* (o) vector of quantizer indexes */
            ...
            if (state_first != 0) {
                ilbc_common.AllPoleFilter(_in, in_idx, weightDenum, weightDenum_idx,
            ...
            for (n = 0;n < len;n++) {
                ...
                if ((state_first != 0) && (n == ilbc_constants.SUBL)) {
                    ...
                    /* synthesis and weighting filters on input */
                    ilbc_common.AllPoleFilter(_in, in_idx + n, weightDenum, weightDenum_idx,
                    ...
                    /* synthesis and weighting filters on input */
                    ilbc_common.AllPoleFilter(_in, in_idx + n, weightDenum, weightDenum_idx, len - n,
                       ilbc_constants.LPC_FILTERORDER);
                ...
                toQ = _in[in_idx + n] - syntOutBuf[syntOut + n];

                sort_sq(index, 0, toQ, ilbc_constants.state_sq3Tbl, 8);
                _out[n] = index[0];
                syntOutBuf[syntOut + n] = ilbc_constants.state_sq3Tbl[_out[n]];
        ...
        private void StateSearchW(
                                  ...
                                  80 vec */
         {
            ...
            System.Array.Copy(residual, residual_idx, tmpbuf, tmp, len);
            ...
            maxVal = (float) System.Math.Log10(maxVal);
            ...
            qmax = (float) System.Math.Pow(10, maxVal);
            scal = 4.5f / qmax;
            for (k = 0;k < len;k++) {
                foutbuf[fout + k] *= scal;
            }
            ...
            float[] a)    /* (i) lpc coefficients */
         {
            ...
            for (lsp_index = 0;lsp_index < ilbc_constants.LPC_FILTERORDER;lsp_index++) {
                ...
                for (step_idx = 0, step = steps[step_idx];
                    ...
                    hlp = (float) System.Math.Cos(omega * ilbc_constants.TWO_PI);
                    ...
                    if (((hlp5 * (olds[old])) <= 0.0f) || (omega >= 0.5)) {
                        ...
                        if (step_idx == (ilbc_constants.LSF_NUMBER_OF_STEPS - 1)) {
                            ...
                            if (System.Math.Abs(hlp5) >= System.Math.Abs(olds[old])) {
            ...
            float[] data)    /* (i) new data vector */
        {
            ...
            _is = ilbc_constants.LPC_LOOKBACK + ilbc_constants.BLOCKL_MAX - this.ULP_inst.blockl;
            ...
            System.Array.Copy(data, 0, this.lpc_buffer, _is, this.ULP_inst.blockl);
            ...
            for (k = 0;k < this.ULP_inst.lpc_n;k++) {
                ...
                _is = ilbc_constants.LPC_LOOKBACK;
                ...
                if (k < (this.ULP_inst.lpc_n - 1)) {
                   ...
                   ilbc_constants.BLOCKL_MAX);
                    ...
                    window(temp, ilbc_constants.lpc_asymwinTbl,
                        this.lpc_buffer, _is, ilbc_constants.BLOCKL_MAX);
                }
            ...
            System.Array.Copy(this.lpc_buffer, ilbc_constants.LPC_LOOKBACK + ilbc_constants.BLOCKL_MAX - _is,
                 this.lpc_buffer, 0, _is);
            ...
            int length)         /* (i) should equate LPC_FILTERORDER */
        {
            ...
            if (this.ULP_inst.mode == 30) {
                ...
                System.Array.Copy(lp, 0, syntdenum, 0, lp_length);
                ...
                for (i = 1;i < this.ULP_inst.nsub;i++) {
                    ...
                    System.Array.Copy(lp, 0, syntdenum, pos, lp_length);
            ...
            if (this.ULP_inst.mode == 30) {
                System.Array.Copy(lsf, lsf2, lsfold, 0, length);
                //            memcpy(lsfold, lsf2, length*sizeof(float));
                System.Array.Copy(lsfdeq, lsfdeq2, lsfdeqold, 0, length);
                //            memcpy(lsfdeqold, lsfdeq2, length*sizeof(float));
            } else {
                System.Array.Copy(lsf, 0, lsfold, 0, length);
                //            memcpy(lsfold, lsf, length*sizeof(float));
                System.Array.Copy(lsfdeq, 0, lsfdeqold, 0, length);
           ...
           int block)           /* (i) the sub-block number */
        {
            ...
            System.Array.Copy(weightState, 0, buf, 0, ilbc_constants.LPC_FILTERORDER);
            //        memcpy(buf,weightState,sizeof(float)*LPC_FILTERORDER);
            System.Array.Copy(mem, mem_idx, buf, ilbc_constants.LPC_FILTERORDER, lMem);
            //        memcpy(buf+LPC_FILTERORDER,mem,lMem*sizeof(float));
            System.Array.Copy(intarget, intarget_idx, buf, ilbc_constants.LPC_FILTERORDER + lMem, lTarget);
            ...
            System.Array.Copy(buf, ilbc_constants.LPC_FILTERORDER + lMem, target, 0, lTarget);
            ...
            for (stage = 0;stage < nStages;stage++) {
                ...
                if ((measure > max_measure) && (System.Math.Abs(ftmp) < ilbc_constants.CB_MAXGAIN)) {
                    best_index = 0;
                    max_measure = measure;
                    gain = ftmp;
                }
...
#pragma warning restore 0162
                ...
                if (stage == 0) {
                    ...
                    if (stage == 1) {
                        gain = ilbc_common.gainquant(gain, System.Math.Abs(gains[stage - 1]),
           ...
           float[] mem)  /* (i/o) the filter state */
       {
        ...
        private static void anaFilter(float[] In, int in_idx, float[] a, int a_idx, int len, float[] Out, int out_idx, float[] mem) {
            ...
            System.Array.Copy(In, in_idx + len - ilbc_constants.LPC_FILTERORDER, mem, 0, ilbc_constants.LPC_FILTERORDER);
        }
        ...
        private static void filteredCBvecs(float[] cbvectors, float[] mem, int mem_idx, int lMem) {
            ...
            for (i = 0;i < ilbc_constants.CB_HALFFILTERLEN;i++)
                tempbuff2[i] = 0.0f;
            System.Array.Copy(mem, mem_idx, tempbuff2, ilbc_constants.CB_HALFFILTERLEN - 1, lMem);
        ...
        private static void searchAugmentedCB(
            ...
            for (icount = low;icount <= high;icount++) {
                ...
                //        System.out.println("ainsi que " + System.Math.Abs(ftmp) + " et " + ilbc_constants.CB_MAXGAIN);

                if ((measure > max_measure[0]) && (System.Math.Abs(ftmp) < ilbc_constants.CB_MAXGAIN)) {
        ...
        private static void createAugmentedVec(int index, float[] buffer, int buffer_idx, float[] cbVec) {
            ...
            pp = buffer_idx - index;
            System.Array.Copy(buffer, pp, cbVec, 0, index);
            ...
            System.Array.Copy(buffer, pp, cbVec, index, ilbc_constants.SUBL - index);
        }
        ...
        public ilbc_encoder(int init_mode) {
            ...
            System.Array.Copy(ilbc_constants.lsfmeanTbl, 0, this.lsfdeqold, 0,
                     ilbc_constants.LPC_FILTERORDER);
            //     for (int li = 0; li < lsfold.Length; li++)
            //         lsfold[li] = 0.0f;

            System.Array.Copy(ilbc_constants.lsfmeanTbl, 0, this.lsfold, 0,
                     ilbc_constants.LPC_FILTERORDER);
        ...
        public short encode(short[] encoded_data, short[] data) {
            ...
            for (k = 0;k < encoded_data.Length;k++)
                encoded_data[k] = (short) (en_data.buffer[2 * k] | (en_data.buffer[2 * k + 1] << 8));
                    ...
                    float[] block)                   /* (o) speech vector to encode */
        {
            ...
            if (state_first != 0) { /* put adaptive part in the end */
                ...
                for (int li = 0;li < ilbc_constants.CB_MEML - this.ULP_inst.state_short_len;li++)
                    mem[li] = 0.0f;
                System.Array.Copy(decresidual, start_pos,
            ...
            if (Nfor > 0) {
                ...
                System.Array.Copy(decresidual, (start - 1) * ilbc_constants.SUBL,
                ...
                for (subframe = 0;subframe < Nfor;subframe++) {
                    ...
                    System.Array.Copy(mem, ilbc_constants.SUBL,
                             mem, 0,
                    ...
                    System.Array.Copy(decresidual, (start + 1 + subframe) * ilbc_constants.SUBL,
            ...
            if (Nback > 0) {
                ...
                for (subframe = 0;subframe < Nback;subframe++) {
                    ...
                    System.Array.Copy(mem, ilbc_constants.SUBL,
                             ...
                             (ilbc_constants.CB_MEML - ilbc_constants.SUBL));
                    //                memcpy(mem, mem+ilbc_constants.SUBL, (ilbc_constants.CB_MEML-ilbc_constants.SUBL)*sizeof(float));
                    System.Array.Copy(reverseDecresidual, subframe * ilbc_constants.SUBL,
                             mem, ilbc_constants.CB_MEML - ilbc_constants.SUBL,
                             ilbc_constants.SUBL);
                    //                memcpy(mem+ilbc_constants.CB_MEML-ilbc_constants.SUBL,
