    ...
    public class ilbc_decoder {
        ...
        private static void syntFilter(
            ...
            float[] mem)      /* (i/o) Filter state */
         {
            ...
            System.Array.Copy(Out, Out_idx + len - ilbc_constants.LPC_FILTERORDER,
        ...
        private void DecoderInterpolateLSF(
            ...
            int length)         /* (i) length of lsf coefficient vector */
         {
            ...
            if (this.ULP_inst.mode == 30) {
                System.Array.Copy(lsfdeq, lsfdeq2, this.lsfdeqold, 0, length);
                //            memcpy(iLBCdec_inst->lsfdeqold, lsfdeq2, length*sizeof(float));
            } else {
                System.Array.Copy(lsfdeq, 0, this.lsfdeqold, 0, length);
             ...
             float period)    /* (i) estimated pitch period */
        {
            ...
            if (st < 0) {
                ...
                System.Array.Copy(idata, 0, vect, -st, (ilbc_constants.ENH_VECTL + st));
                //         memcpy(&vect[-st],idata, (ilbc_constants.ENH_VECTL+st)*sizeof(float));
            } else {
                en = st + ilbc_constants.ENH_VECTL;

                if (en > idatal) {
                    System.Array.Copy(idata, st, vect, 0, (ilbc_constants.ENH_VECTL - (en - idatal)));
                    ...
                    System.Array.Copy(idata, st, vect, 0, ilbc_constants.ENH_VECTL);
               ...
               float alpha0)/* (i) max smoothing energy fraction */
        {
            ...
            for (i = 1;i <= 2 * hl + 1;i++) {
                wt[i - 1] = (float) 0.5 * (1 - (float) System.Math.Cos(2 * ilbc_constants.PI * i / (2 * hl + 2)));
            }
            ...
            if (System.Math.Abs(w11) < 1.0f) {
                w11 = 1.0f;
            }
            C = (float) System.Math.Sqrt(w00 / w11);
            ...
            if (errs > alpha0 * w00) {
                ...
                if (denom > 0.0001f) { /* eliminates numerical problems
                     for if smooth */
                    A = (float) System.Math.Sqrt((alpha0 - alpha0 * alpha0 / 4) / denom);
             ...
             int hl)              /* (i) 2*hl+1 is the number of sequences */
        {
            ...
            System.Array.Copy(idata, centerStartPos, sseq, psseq, ilbc_constants.ENH_BLOCKL);
                  ...
                  float[] _in)                      /* (i) unenhanced signal */
        {
            ...
            System.Array.Copy(enh_buf, this.ULP_inst.blockl,
                     enh_buf, 0,
            ...
            System.Array.Copy(_in, 0, enh_buf, ilbc_constants.ENH_BUFL - this.ULP_inst.blockl,
            ...
            i = 3 - ioffset;
            System.Array.Copy(enh_period, i, enh_period, 0, ilbc_constants.ENH_NBLOCKS_TOT - i);
            ...
            System.Array.Copy(enh_buf, (ilbc_constants.ENH_NBLOCKS_EXTRA + ioffset) * ilbc_constants.ENH_BLOCKL - 126,
                     lpState, 0, 6);
            //     memcpy(lpState,
            ...
            if (this.prev_enh_pl == 1) {
                ...
                lag = inlag - 1;
                maxcc = xCorrCoef(_in, 0, _in, lag, plc_blockl);
                for (ilag = inlag;ilag <= inlag + 1;ilag++) {
                    cc = xCorrCoef(_in, 0, _in, ilag, plc_blockl);
                    if (cc > maxcc) {
                        maxcc = cc;
                        lag = ilag;
                    }
                ...
                for (isample = start;isample > 0;isample--) {
                    //        *enh_bufPtr1-- = *inPtr--;
                    plc_pred[enh_bufPtr1] = _in[inPtr];
                    enh_bufPtr1--;
                    inPtr--;
                }
                ...
                ftmp1 = (float) System.Math.Sqrt(ftmp1 / plc_blockl);
                ftmp2 = (float) System.Math.Sqrt(ftmp2 / plc_blockl);
            ...
            if (this.ULP_inst.mode == 20) {
                ...
                for (iblock = 0;iblock < 2;iblock++) {
                    enhancer(_out, iblock * ilbc_constants.ENH_BLOCKL, enh_buf,
                 ...
                 int sRange)      /* (i) correlation search length */
        {
            ...
            if (ftmp2 > 0.0f) {
                ...
                gc[0] = System.Math.Abs(ftmp1 / ftmp2);
                pm[0] = System.Math.Abs(ftmp1) /
                ((float) System.Math.Sqrt(ftmp2) * (float) System.Math.Sqrt(ftmp3));
            } else {
                cc[0] = 0.0f;
                gc[0] = 0.0f;
                pm[0] = 0.0f;
                 ...
                 int inlag)          /* (i) pitch lag */
        {
            ...
            if (PLI == 1) {
                ...
                ftmp = (float) System.Math.Sqrt(max_per);
                if (ftmp > 0.7f)
                    pitchfact = 1.0f;
                ...
                if ((float) System.Math.Sqrt(energy / this.ULP_inst.blockl) < 30.0f) {
                    gain = 0.0f;
                    for (i = 0;i < this.ULP_inst.blockl;i++) {
                ...
                System.Array.Copy(this.prevLpc, 0, PLClpc, 0, ilbc_constants.LPC_FILTERORDER + 1);
            ...
            /* no packet loss, copy input */
                ...
                System.Array.Copy(decresidual, 0, PLCresidual, 0, this.ULP_inst.blockl);
                //        memcpy(PLClpc, lpc, (LPC_FILTERORDER+1)*sizeof(float));
                System.Array.Copy(lpc, lpc_idx, PLClpc, 0, ilbc_constants.LPC_FILTERORDER + 1);
                this.consPLICount = 0;
            }
            ...
            System.Array.Copy(PLClpc, 0, this.prevLpc, 0, ilbc_constants.LPC_FILTERORDER + 1);
            //    memcpy(this.prevResidual, PLCresidual, this.ULP_inst.blockl*sizeof(float));
            System.Array.Copy(PLCresidual, 0, this.prevResidual, 0, this.ULP_inst.blockl);
        }
        ...
        public ilbc_decoder(int init_mode, int init_enhancer) {
            ...
            System.Array.Copy(ilbc_constants.lsfmeanTbl, 0, lsfdeqold, 0,
